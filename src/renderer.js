...
function renderTable() {
  ...
  currentFiles.forEach((file, index) => {
	...
	tr.addEventListener('dragstart', (e) => {
	  // ドラッグ開始した項目が選択されていなければ、単一選択にし直す
	  if (!selectedIndices.has(index)) selectImage(index);
	  
	  const paths = Array.from(selectedIndices).map(i => currentFiles[i].path);
	  e.dataTransfer.setData('application/json', JSON.stringify(paths));
	  e.dataTransfer.setData('text/plain', paths[0]); // 外部アプリ等へのフォールバック
	  e.dataTransfer.effectAllowed = 'copyMove';
	  e.dataTransfer.setDragImage(emptyDragImage, 0, 0); // OS標準の半透明のゴースト画像を消す
...
function renderGrid() {
  ...
  thumbnailGrid.style.gap = '8px'; // 常に一定の余白を保つ
  thumbnailGrid.style.justifyContent = 'flex-start'; // 左詰めで配置
  thumbnailGrid.style.alignContent = 'flex-start'; // 縦方向にも上詰めで配置
  ...
  const processNextChunk = (chunkIndex) => {
    ...
    requestIdleCallback(async () => {
      ...
      try {
        ...
        // 取得したメタデータでcurrentFilesとUI（テーブル）を更新
        metadataList.forEach(meta => {
          const fileIndex = currentFiles.findIndex(f => f.path === meta.path);
          if (fileIndex > -1) {
